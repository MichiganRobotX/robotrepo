...
void handleFrame(const nav_msgs::Odometry &msg) {
    ...
    for(int i=1; i<=5; i++) {
        ...
        if(i==1) {
            ...
            br.sendTransform(tf::StampedTransform(transform, time, "world", "map"));
        }
        // map to base_link
        if(i==2) {
            transform.setOrigin(tf::Vector3(qx, qy, 0.5));
            transform.setRotation(tf::Quaternion(0, 0, 0, 1));
            br.sendTransform(tf::StampedTransform(transform, time, "map", "base_link"));
        }
        // base_link to gps
        if(i==3) {
            transform.setOrigin(tf::Vector3(0, 0, 0));
            transform.setRotation(tf::Quaternion(0, 0, 0, 1));
            br.sendTransform(tf::StampedTransform(transform, time, "base_link", "gps"));
        }
        // velodyne to base_link
        if(i==4) {
            transform.setOrigin(tf::Vector3(2.4, 0, 1.8));
            transform.setRotation(tf::Quaternion(0, 0, 0, 1));
            br.sendTransform(tf::StampedTransform(transform, time, "base_link", "velodyne"));
        }
        // base_link to odom
        if(i==5) {
            transform.setOrigin(tf::Vector3(0, 0, 0));
