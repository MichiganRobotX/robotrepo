...
void msy_multi_lidar::lidar_read3( const sensor_msgs::PointCloud2ConstPtr & laserCloudMsg){
    ...
    pcl::fromROSMsg(*laserCloudMsg, CloudTL_ni);
}
...
sub3 = nh.subscribe("vel2/velodyne_points", 10, &msy_multi_lidar::lidar_read3, this);
pub3 = nh.advertise<sensor_msgs::PointCloud2>("/transferred_TL", 10);
sub4 = nh.subscribe("vel1/velodyne_points", 10, &msy_multi_lidar::lidar_read4, this);
...
pub_calib = nh.advertise<sensor_msgs::PointCloud2>("/calib", 10);
       ...
       pcl::PointCloud<pcl::PointXYZI> & Source_surf)
{
  ...
  if (lastCornerCloudSize > 10 && lastSurfaceCloudSize > 100) {
    ...
    for (size_t iterCount = 0; iterCount < _maxIterations; iterCount++) {
      ...
      transform_rx += matX(0, 0) * sss;
      transform_ry += matX(1, 0) * sss;
      transform_rz += matX(2, 0) * sss;
      transform_tx += matX(3, 0) * sss;
      transform_ty += matX(4, 0) * sss;
      transform_tz += matX(5, 0) * sss;
      ...
      float _deltaTAbort = 0.03;     ///< optimization abort threshold for deltaT
      float _deltaRAbort = 0.005;     ///< optimization abort threshold for deltaR
...
bool msy_multi_lidar::process()
{
  ...
  pcl::PointCloud<pcl::PointXYZI> PC_Source_corner_TR;
  pcl::PointCloud<pcl::PointXYZI> PC_Source_surf_TR;

  pcl::PointCloud<pcl::PointXYZI> PC_Source_corner_TL;
  pcl::PointCloud<pcl::PointXYZI> PC_Source_surf_TL;

  Registration(CloudTC_ni, _timeTC, PC_Target_corner, PC_Target_surf);
  Registration(CloudTR_ni, _timeTR, PC_Source_corner_TR, PC_Source_surf_TR);
  Registration(CloudTL_ni, _timeTL, PC_Source_corner_TL, PC_Source_surf_TL);

  // std::cout << "============================================================" << std::endl;
  // std::cout << "============================TR to TC========================" << std::endl;
  // std::cout << "============================================================" << std::endl;
  // LM(PC_Target_corner, PC_Target_surf, PC_Source_corner_TR, PC_Source_surf_TR);

  std::cout << "============================================================" << std::endl;
  std::cout << "============================TL to TC========================" << std::endl;
  std::cout << "============================================================" << std::endl;
  LM(PC_Target_corner, PC_Target_surf, PC_Source_corner_TL, PC_Source_surf_TL);
